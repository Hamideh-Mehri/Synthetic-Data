...
class DataTransformer(object):
    ...
    def _synchronous_transform(self, raw_data, column_transform_info_list):
        ...
        return column_data_list, mean_dict, std_dict,df


    def transform(self, raw_data):
        """Take raw data and output a matrix data."""
        column_data_list, mean_dict, std_dict, df = self._synchronous_transform(raw_data,self._column_transform_info_list)
       
        return np.concatenate(column_data_list, axis=1).astype(float), mean_dict, std_dict, df
    ...
    def generate_raw_ps(self, data):
        ...
        for column_transform_info in self._column_transform_info_list:
            ...
            st += dim
        recovered_data = np.column_stack(recovered_column_data_list)
        recovered_data = (pd.DataFrame(recovered_data, columns=column_names).astype(self._column_raw_dtypes.loc[['amount','tcode']]))

        return recovered_data, day_ps, dtme_ps, dow_ps, month_ps, gaussian_component
