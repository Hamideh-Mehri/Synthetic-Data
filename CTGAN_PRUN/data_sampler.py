...
class DataSampler(object):
    ...
    def sample_original_condvec3(self, idx):
        """Generate the conditional vector for generation use original frequency."""
        if self._n_discrete_columns == 0:
            return None

        cond = np.zeros((len(self.transactions[idx]), self._n_categories), dtype='float32')
        

        for i in range(len(self.transactions[idx])):
            row_idx = i
            col_idx = np.random.randint(0, self._n_discrete_columns)               #len(transaction) = number of unique account id's
            matrix_st = self._discrete_column_matrix_indexes[col_idx]             #corrected by myself
            matrix_ed = matrix_st + self._discrete_column_n_category[col_idx]
